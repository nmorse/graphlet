...
test( "Graphel selector tests", function( assert ) {
 ...
 "edges":[
  ...
  assert.deepEqual( 
	gq.using(graph).find({"element":"node", "id":"n1"}),  
	[{"name":"send","process":["~salutation = ~salutation . ~name"],"data":{"salutation":"Hi "},"id":"n1","view":{"position":{"x":124,"y":196}},"node_type":"process"}],
	"selection of node n1" 
  );
  assert.deepEqual( 
	gq.using(graph).find({"element":"edge", "type":"get", "from":"n1"}),  
  [["n1","n2","get","salutation",null],
  ["n1","n4","get","name",null]],
	"get edges out node n1" 
  );
});
